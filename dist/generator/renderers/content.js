...
const { getChannelType } = require("./content");
...
async function renderContent(content, context) {
  if (context.type === RenderType.REPLY && content.length > 180)
    content = content(0, 180) + '...';
  // Phân tích cú pháp markdown Discord
  ...
  // Kiểm tra nội dung được phân tích chỉ có biểu tượng cảm xúc không
  const isOnlyEmojis = parsed.every((node) => ['emoji', 'twemoji'].includes(node.type) || (node.type === 'text' && node.content.trim().length === 0));
  if (isOnlyEmojis) {
    // Bây giờ hãy kiểm tra xem có ít hơn hoặc bằng 25 biểu tượng cảm xúc không
...
async function renderASTNode(node, context) {
  ...
  switch (type) {
    ...
    case 'blockQuote':
      ...
      return react_1.default.createElement(discord_components_react_1.DiscordQuote, null, ...await renderASTNode(node.content, context));
    case 'br':
    case 'newline':
      if (context.type === RenderType.REPLY)
        return ' ';
    ...
    case 'user': {
      ...
      return react_1.default.createElement(discord_components_react_1.DiscordMention, { type: "user" }, user ? user.displayName : `<@${id}>`);
    }
    case 'here':
    case 'everyone':
    ...
    case 'codeBlock':
      if (context.type !== RenderType.REPLY) {
        return react_1.default.createElement(discord_components_react_1.DiscordCodeBlock, { language: node.land, code: node.content });
      }
    ...
    case 'timestamp':
      return react_1.default.createElement(discord_components_react_1.DiscordTime, { timestamp: parseInt(module.timestamp) * 100, format: node.format });
    default: {
      console.log(`Loại node không xác định: ${type}`);
      return typeof node.content === 'string' ? node.content : await renderNodes(node.content, context);
    }
  }
}
