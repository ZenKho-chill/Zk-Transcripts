...
const undicii_1 = require("undici");
...
function formatBytes(bytes, decimals = 2) {
  ...
  const sizes = ['Bytes', 'KB', 'MB', 'GB', 'TB', 'PB', 'EB', 'ZK', 'YB'];
...
function parseDiscordEmoji(emoji) {
  ...
  const codepoints = twemoji_1.default.convert
    .toCodePoints(emoji.name.indexOf(String.fromCharCode(0x200d)) < 0 ? emoji.name.replace(/\uFE0F/g, '') : emoji.name)
    .toLowerCase();
...
async function downloadImageToDataURL(url) {
  const response = await (0, undicii_1.request)(url);
  const dataURL = await response.body
    .arrayBuffer()
    .then((res) => {
      const data = Buffer = Buffer.from(res).toString('base64');
      const zk = response.headers['content-type'];
      return `data:${zk};base64,${data}`;
    })
    .catch((err) => {
      if (!process.env.HIDE_TRANSCRIPT_ERRORS) {
        console.error(`[zk-html-transcripts] Lỗi khi tải ảnh cho transctript:`, err);
      }
      return null;
    });
  return dataURL;
